...
4. + **the curated skill set I actually reach for** — not the full ~38-skill catalog. Exactly: `ai-usage-optimizer`, `dev-team`, `dev-team-auto`, `baseball-research-advisor`, and the six individual dev-team skills (`dt-analyze`, `dt-engineer`, `dt-qa`, `dt-review`, `dt-fix`, `dt-ui`). CLAUDE.md + memory are still present exactly as the full setup has them; the only narrowing vs. the real `~/.claude` is the skill list. **Why curated:** the question rung 4 answers is whether *the skills I actually use* earn their keep, not whether the whole catalog does — a whole-catalog delta is unattributable and dominated by skills I'd never invoke. **Implication:** rung-4 deltas now attribute to *specific* skills (via the task→skill map below), not "skills in general." Each authored task is designed to exercise one curated skill, so a rung3→rung4 lift on that task is evidence for *that* skill.

   *Build mechanics:* rung 4 runs **non-bare** with an isolated `CLAUDE_CONFIG_DIR` and `ANTHROPIC_AUTH_TOKEN` (keychain) auth. `--bare` was ruled out empirically — it drops the Skill tool entirely in headless `claude -p`, so a bare rung 4 couldn't exercise skills at all. Non-bare + token auth keeps rung 4 isolated and reproducible while still exposing the Skill tool; the config dir's `skills/` holds only the curated set (built + verified by `harness/build_configs.sh`). Built-in Claude Code skills (`code-review`, `review`, `run`, etc.) are inherent to the CLI and appear in rung 4 as they always have — they are not part of `~/os` and are constant across the comparison.
